<template>
  <div id="updataAvatar">
    <el-avatar :size="100" 
    :src="user_avatar?'http://192.168.43.177:8081/'+user_avatar:'https://cube.elemecdn.com/3/7c/3ea6beec64369c2642b92c6726f1epng.png'"
    ></el-avatar>
    <el-button
        type="success"
        plain
...
export default {
  ...
  methods: {
    ...
    cropSuccess(imgDataUrl, field) {
      ...
      this.postRequest('/api/sendImg/uploadImg',{Base64Str:newImgUrl}).then(res=>{
        if(res.data.code==0){
          this.cropUploadSuccess(res.data.imgSrc);
          // imgDataUrl是经过base64编码的图像
          // 裁剪成功修改父组建头像的值
          this.$emit('updataAvatar',{imgUrl:imgDataUrl,imgUrl2:res.data.imgSrc})
        }
      })
